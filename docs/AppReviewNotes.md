...
[앱과 기능 설명]
바쁜 일과 시간 중에도 말씀과 멀어지지 않고자 노력하는 크리스천을 위한 앱입니다.
이 앱은 iOS의 Live Activities (실시간 현황) 기능을 사용하여
성경 말씀과 함께 걸음 수를 잠금화면에 표시합니다.


핵심 기술:
- iOS ActivityKit (Live Activities): iPhone 14+ Dynamic Island 지원
- Core Motion (CMPedometer) 연동: 사용자의 일일 걸음 수를 실시간으로 추적
...
1. 권한 허용 시:
   ...
   - 앱이 백그라운드에서도 Core Motion 데이터 감시
...
1. 일반 테스트:
   - 앱 설치 후 "시작" 버튼 클릭
   - 성경 말씀 1개 선택
   - 동작 및 피트니스 권한 팝업 → "확인" 선택
   - 잠금화면 위젯에 구절 + 👣 걸음 수 표시 확인
...
3. 동작 및 피트니스 권한 거절 테스트:
   - 앱 완전 삭제 후 재설치
   - "시작" 클릭 후 동작 및 피트니스 권한 팝업 → "허용 안 함" 선택
   - 30초 대기 후 잠금화면 확인
   - 👣(걸음) 대신 ⏱️(타이머) 표시됨
   - 타이머가 초 단위로 계속 증가함
...
- 프레임워크: ActivityKit, Core Motion (CMPedometer), UserDefaults
- 최소 지원 버전: iOS 16.2
...
- Motion & Fitness (Core Motion): Real-time daily step count tracking
...
[Motion & Fitness Permission Handling]

1. Permission Allowed:
   - Real-time step count updates on lock screen
   - App monitors Motion data in background using CMPedometer.startUpdates()
...
- Core Motion reads device-local data only
...
1. Basic Functionality:
   ...
   - When Motion & Fitness permission popup appears: tap "OK"
   - Wait 30 seconds
...
3. Motion & Fitness Permission Denied Scenario:
   - Completely delete app
   - Reinstall fresh
   - Tap "Start Session", select verse
   - When Motion & Fitness permission popup appears: tap "Don't Allow"
   - Wait 30 seconds
...
- Frameworks: ActivityKit, Core Motion (CMPedometer), UserDefaults
- Minimum iOS Version: 16.2

[Additional Notes for Reviewers]

- This app uses Core Motion (CMPedometer) API for "real-time changing user activity data,"
...
- Privacy Assurance: All Motion & Fitness data is read locally on-device.
...
- Soft Prompt UX: Before requesting Motion & Fitness permission, app shows
...
| 항목 | 심사팀 질문 | 우리의 답변 |
|------|-----------|----------|
| **Core Motion 사용 정당성** | 왜 걸음 수를 읽나? | "실시간 사용자 활동 표시"라고 명시 |
